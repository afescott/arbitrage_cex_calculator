use crate::{
    api::execution::{submit_limit_order, LimitOrderRequest, OrderSide},
    args::Args,
...
impl PurchaseManager {
    ...
    pub async fn run_purchase_simulation(&mut self) {
        while let Some(route) = self.rx.recv().await {
            ...
            if self.args.hyperliquid_private_key.is_none() {
                eprintln!("Missing `--hyperliquid-private-key`; skipping execution");
                continue;
            }
            if self.args.binance_api_key.is_none() || self.args.binance_api_secret.is_none() {
            ...
            let _ = submit_limit_order(LimitOrderRequest {
